...
def compute_similarity(image_pair):
    ...
    # Print image paths for debugging purposes
...
if __name__ == '__main__':
    ...
    similar_images = [(image_pair[0], image_pair[1], similarity) for image_pair, similarity in similarity_matrix.items() if similarity > threshold]

    print(similar_images)

    # Benzer görüntü çiftlerini yazdırma
    if len(similar_images) > 0:
        print("Similar image pairs found:")
        for image_pair in similar_images:
            print(f"Similar images: {image_pair[0]} and {image_pair[1]}, similarity: {round(image_pair[2], 6)}")
    else:
